...
void RefreshMupens(BOOL msg)
{
    ...
    char buf[32];
    sprintf_s(buf,sizeof(buf),"Disconnected");
    if (hMupenwnd.size()!=0)
    {
        sprintf_s(buf,sizeof(buf),"Found %i mupen instance(s)",hMupenwnd.size());
        MessageBox(0,buf,"Info",64);
        sprintf_s(buf,sizeof(buf),"%i connected",hMupenwnd.size());
    }
    else if(msg)
    {
...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    char txt[200] = "Using: ";
    strcat_s(txt,sizeof(txt),PluginInfo.Name);
